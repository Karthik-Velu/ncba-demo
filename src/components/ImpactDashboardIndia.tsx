...
// ---- helpers ----
function fmtNum(n: number, currency: 'INR' | 'USD'): string {
  const v = currency === 'USD' ? n / INR_TO_USD : n;
  const prefix = currency === 'USD' ? '$' : '₹';
  if (Math.abs(v) >= 1e9) return `${prefix}${(v / 1e9).toFixed(2)}B`;
  if (Math.abs(v) >= 1e7) return `${prefix}${(v / 1e7).toFixed(2)} Cr`;
  if (Math.abs(v) >= 1e5) return `${prefix}${(v / 1e5).toFixed(1)} L`;
...
const PRODUCT_ICONS: Record<string, ReactNode> = {
  'E-2W':       <Zap className="w-3.5 h-3.5 text-green-600" />,
  'E-3W':       <Truck className="w-3.5 h-3.5 text-teal-600" />,
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] text-gray-400 font-medium ml-1">Kaleidofin India Climate Taxonomy · MDB/IDFC Common Principles · Paris Aligned</span>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={climateBalanceData}>
                ...
                <YAxis tick={{ fontSize: 9 }} tickFormatter={v => currency === 'USD' ? `$${(v / INR_TO_USD / 1e6).toFixed(1)}M` : `${(v / 1e6).toFixed(0)}M`} />
      ...
      <div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-4">
          ...
          <InfoBar>
            <strong>E-2W:</strong> Electric two-wheeler vs petrol scooter. Net: ~0.52 tCO₂e/loan/yr. IPCC AR6 + CEA India grid (2023).<br />
            <strong>E-3W:</strong> Electric three-wheeler vs CNG/diesel auto. Net: ~1.54 tCO₂e/loan/yr. MDB activity 8.6.<br />
            <strong>EV:</strong> Electric 4WD vs petrol equivalent. Net: ~2.02 tCO₂e/loan/yr. MDB Common Principles activity 8.6.<br />
            <strong>Solar Home:</strong> Displaces kerosene lighting and diesel genset. Net: ~0.32 tCO₂e/loan/yr. MNRE Off-grid Solar Programme.<br />
            <strong>Solar Pump:</strong> Displaces diesel irrigation pump. Net: ~2.01 tCO₂e/loan/yr. MNRE PM-KUSUM evaluation (MDB activity 7.3).<br />
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] text-gray-400 font-medium ml-1">Agri · MSME · SHG/JLG · Kharif–Rabi aligned · risk-based pricing</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        ...
        <div className="grid grid-cols-2 gap-5 mb-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <div className="space-y-2.5">
              {[
                { label: 'Women Smallholders (SHG/JLG Agri)',   count: resBreakdown.womenAgriCount,   balance: resBreakdown.womenAgriBalance,   color: 'bg-pink-400',  desc: 'SHG / JLG Agri (women proxy)' },
                { label: 'Irrigation / Solar Pump (≥ ₹100K Agri)', count: resBreakdown.irrigationCount,  balance: resBreakdown.irrigationBalance,  color: 'bg-blue-400',  desc: 'Larger Agri — PM-KUSUM, drip irrigation' },
                { label: 'Farm Input Loans (< ₹50K Agri)',         count: resBreakdown.farmInputCount,   balance: resBreakdown.farmInputBalance,   color: 'bg-teal-400',  desc: 'Small agri-input finance: seeds, fertilizer' },
                { label: 'General Agri (₹50K–100K)',               count: resBreakdown.generalAgriCount, balance: resBreakdown.generalAgriBalance, color: 'bg-green-400', desc: 'Mid-range agri credit' },
                ...
                { label: 'SHG / JLG',                            count: resBreakdown.shgCount,         balance: resBreakdown.shgBalance,         color: 'bg-violet-400', desc: 'Self-Help Group / Joint Liability Group' },
              ].map(row => (
                <div key={row.label}>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              ...
              <p className="text-[9px] text-gray-400 mt-2">India Kharif sowing: Jun–Jul (pre-sowing Apr–Jun). Rabi sowing: Oct–Nov (pre-sowing Sep–Oct). Aligned disbursements improve repayment performance.</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <h4 className="text-xs font-bold text-[#003366] mb-3 flex items-center gap-1.5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          ...
          <ImpactKpiCard label="Small-Holder Farmers" value={vulBreakdown.smallHolderCount.toLocaleString()} sub="Agri loan ≤ ₹30K (high risk)" icon={<Sprout className="w-4 h-4 text-orange-500" />} accent="border-l-orange-400" />
          <ImpactKpiCard
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <div className="space-y-3">
              <div className="bg-orange-50 rounded-lg p-3">
                <p className="text-[10px] text-orange-600 mb-1 font-semibold">Definition: Agri-Finance loan ≤ ₹30,000 in high-risk geography (marginal 1–2 acre farmer proxy)</p>
              ...
              <p className="text-[9px] text-gray-500 leading-relaxed">
                Small-holder farmers in drought-prone regions (Marathwada, Bundelkhand, Vidarbha) and flood-prone regions (North Bihar, Sundarbans) face amplified default risk during climate events. PMFBY crop insurance enrolment and proactive engagement significantly reduce delinquency.
              </p>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
          ...
          <ImpactKpiCard label="Low Income Group" value={pct(socio.lowIncomePct)} sub={`Loan < ${currency === 'USD' ? `$${(50000 / INR_TO_USD).toFixed(0)}` : '₹50K'}`} icon={<Shield className="w-4 h-4 text-indigo-500" />} accent="border-l-indigo-400" />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-green-50 rounded-lg p-3 text-center">
                <p className="text-xl font-bold text-green-700">{fmtNum(socio.estimatedAnnualIncomeImpactINR, currency)}</p>
              ...
              <div className="bg-blue-50 rounded-lg p-3 text-center">
                <p className="text-xl font-bold text-blue-700">
                  {socio.totalBorrowers > 0 ? fmtNum(socio.estimatedAnnualIncomeImpactINR / socio.totalBorrowers, currency) : '—'}
                </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-teal-50 rounded-lg p-3 text-center">
                <p className="text-xl font-bold text-teal-700">{fmtNum(socio.avgLoanSizeINR, currency)}</p>
                <p className="text-[10px] text-gray-500 mt-0.5">Average loan size</p>
                <p className="text-[10px] text-gray-400">disbursed amount</p>
              </div>
            </div>
            <InfoBar>
              <strong>Income multipliers (additional annual income per ₹ deployed, conservative CGAP/IFC proxies):</strong><br />
              E-3W 1.4× · E-2W 1.0× · EV 1.0× · MSME 0.9× · Solar-Pump 0.7× · Dairy 0.6× · Agri-Finance 0.4× · SHG 0.4× · JLG 0.35× · Solar-Home 0.15×<br />
              <strong>NTC proxy:</strong> SHG/JLG/Individual segment with loan &lt; ₹10,000 — first-time micro-credit access.<br />
              <strong>Micro-entrepreneur proxy:</strong> MSME product or Enterprise segment with loan &lt; ₹2,50,000.<br />
              <strong>Jobs proxy:</strong> MSME/Enterprise 1.8 jobs/loan; E-2W/E-3W/EV 1.0; Agri-Finance 1.2; Solar-Pump 1.3; Dairy 1.1; others 0.5.<br />
      ...
      <div>
        ...
        {(() => {
          const highRiskLoans = loans.filter(l => ['Marathwada','Bundelkhand','Vidarbha','North Bihar','Sundarbans'].includes(l.geography ?? ''));
          ...
          return (
            <p className="text-[10px] text-gray-500 mb-4">
              <span className="font-semibold text-gray-700">{((highRiskLoans.length / loans.length) * 100).toFixed(1)}%</span> of portfolio in high climate-risk geographies (Marathwada, Bundelkhand, Vidarbha, North Bihar, Sundarbans)
              {' · '}
        ...
        {geoProbs.length > 0 && (
          <div>
            ...
            <p className="text-[10px] text-gray-500 mb-3">
              12-month rolling probability scores derived from historical climate variables (IMD, NRSC Bhuvan, IIT Gandhinagar Drought Atlas, NOAA CPC ENSO/IOD, CWC flood data).
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-x-auto">
              ...
              <div className="px-3 py-2 bg-gray-50 border-t border-gray-100">
                <p className="text-[9px] text-gray-400">
                  ...
                  Sources: IMD (India Meteorological Department), NRSC (National Remote Sensing Centre) Bhuvan, IIT Gandhinagar Drought Atlas, NOAA CPC ENSO/IOD, CWC (Central Water Commission).
                </p>
              </div>
            </div>
      ...
      <div>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] text-gray-400 ml-1">Predictive · External climate data · IMD / NRSC / NOAA CPC</span>
          </div>
          {tierOneEWI.length > 0 ? (
