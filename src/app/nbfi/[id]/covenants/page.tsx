...
export default function CovenantsPage() {
  ...
  useEffect(() => {
    if (tabFromUrl === 'early-warnings') setActiveTab('early-warnings');
  }, [tabFromUrl]);
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8 overflow-auto bg-gray-50">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-bold text-gray-800">Covenant & Early Warnings — {nbfi.name}</h1>
            <p className="text-sm text-gray-500 mt-1">
              {activeTab === 'covenants' ? 'Real-time covenant compliance and provisioning (read-only)' : 'Predictive monitoring and trend analysis'}
            </p>
          </div>
          <div className="flex gap-2">
            ...
            {activeTab === 'covenants' && (
              <>
                <Link href={`/nbfi/${id}/setup`} className="flex items-center gap-2 px-4 py-2 border border-[#003366] text-[#003366] rounded-lg text-sm font-medium hover:bg-blue-50 transition-colors">
                  <Eye className="w-4 h-4" /> Edit Covenants (Step 3)
                </Link>
                <button onClick={() => setShowReview(!showReview)} className="flex items-center gap-2 px-4 py-2 bg-[#003366] text-white rounded-lg text-sm font-medium hover:bg-[#004d99] transition-colors">
                  <FileBarChart className="w-4 h-4" /> {showReview ? 'Hide' : 'Generate'} Quarterly Review
                </button>
              </>
            )}
          </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  <table className="w-full text-xs">
                    <thead><tr className="border-b border-gray-200"><th className="text-left py-2 font-medium text-gray-500">Metric</th><th className="text-right py-2 font-medium text-gray-500">Projected Value</th><th className="text-right py-2 font-medium text-gray-500">Status</th></tr></thead>
                    <tbody>
